...
长 agent 回合会在模型回复之间穿插大量工具调用与结果。每条 `assistant-message` 卡片右侧都有一个折叠开关；点击会隐藏该回复与下一条模型回复之间的所有内容，让时间轴只呈现对话的回复主干，把工具执行的中间过程收起来。

折叠是**就地视图变换**，而非重新布局：

- 时间轴始终基于完整（未折叠）快照布局，因此列的分配、节点坐标与当前的平移/缩放状态在折叠/展开前后都保持不变。
- 被隐藏的工具调用与结果离开画布；被折叠的回复正下方会出现一个**折叠胶囊**，标注隐藏节点数量，并用虚线箭头把该回复桥接到下一条模型回复。
- 点击胶囊（或回复卡片上的开关，此时已指向展开）即可恢复该段。

只有确实带有后续工具节点的模型回复才可以折叠——没有工具段的回复不显示开关。折叠按模型回复独立进行：折叠某条回复绝不会隐藏另一条的段。段归属按时间顺序推导（`assistantFoldMembers`），隐藏集合与胶囊由 `foldCapsuleView` 纯函数计算。
